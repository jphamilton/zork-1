...
{
    private List<X> _multiTypes = [X.Multi, X.MultiHeld, X.MultiExcept, X.MultiInside];
    private List<X> _requires = [];

    public List<string> Verbs { get; protected set; } = [];
    
    public List<string> Prepositions { get; protected set; } = [];
    
    public List<X> Requires
    {
        get => _requires;
        protected set
        {
            ...
            if (value.Contains(X.None))
            {
                throw new Exception($"[{GetType().Name}]: Object type X.None is reserved");
            }
    ...
    protected Room Location => Context.Current.Location;
    
    protected bool Lit => State.Lit;

    public bool GoTo<T>() where T : Room
    {
        return MovePlayer.To<T>();
    }
